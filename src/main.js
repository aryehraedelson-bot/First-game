...
const player = { name: 'Kai', age: 15, species: 'Golden Dragon', x: 90, y: 330, w: 34, h: 58, vx: 0, vy: 0, hp: 100, maxHp: 100, facing: 1, grounded: false, coyote: 0, jumpBuffer: 0, attackBuffer: 0, attacking: 0, combo: 0, comboWindow: 0, dodge: 0, invincible: 0, lightning: false, lightningCd: 0, rescued: 0 };
...
const feedback = { hitStop: 0, shake: 0 };
...
function update(dt) {
  world.timer += dt; feedback.shake = Math.max(0, feedback.shake - dt); player.lightningCd = Math.max(0, player.lightningCd - dt); player.comboWindow = Math.max(0, player.comboWindow - dt); player.attacking = Math.max(0, player.attacking - dt); player.attackBuffer = Math.max(0, player.attackBuffer - dt); player.dodge = Math.max(0, player.dodge - dt); player.invincible = Math.max(0, player.invincible - dt); player.jumpBuffer = Math.max(0, player.jumpBuffer - dt);
  if (feedback.hitStop > 0) { feedback.hitStop = Math.max(0, feedback.hitStop - dt); return; }
  ...
  if (tapped('j')) player.attackBuffer = 8;
  if (player.attackBuffer > 0 && player.attacking <= 0) staffAttack();
...
function staffAttack() { player.attackBuffer = 0; player.attacking = 10; player.combo = player.comboWindow > 0 ? Math.min(3, player.combo + 1) : 1; player.comboWindow = 30; hitThings(52 + player.combo * 12, 16 + player.combo * 7); }
function lightningStrike() { player.attackBuffer = 0; player.attacking = 18; player.lightningCd = 90; hitThings(115, 75); world.message = 'Kai channels Lightning Strike through the Golden Bo Staff!'; }
function hitThings(range, dmg) {
  const ax = player.x + player.w / 2 + player.facing * range / 2;
  let landed = false;
  for (const e of enemies) if (e.alive && overlap(ax - range / 2, player.y, range, player.h, e.x, e.y, e.w, e.h)) {
    e.hp = Math.max(0, e.hp - dmg);
    e.hurt = 10;
    e.attack = Math.max(e.attack, 14);
    e.vx = player.facing * 3.2;
    landed = true;
    if (e.hp <= 0) { e.alive = false; e.vx = 0; }
  }
  if (boss.active && !boss.defeated && overlap(ax - range / 2, player.y, range, player.h, boss.x, boss.y, boss.w, boss.h)) {
    boss.hp = Math.max(0, boss.hp - dmg);
    boss.hurt = 12;
    boss.windup = Math.max(boss.windup, 22);
    landed = true;
    if (boss.hp <= 0) { boss.defeated = true; boss.active = false; objects.find(o => o.type === 'relic').active = true; world.objective = 'Absorb the Lightning Relic'; world.message = 'The Fire Beast falls. The Lightning Relic remains behind.'; }
  }
  if (landed) { feedback.hitStop = 2; feedback.shake = 7; }
}
function updateEnemies(dt) { for (const e of enemies) { if (!e.alive) continue; e.hurt = Math.max(0, e.hurt - dt); e.attack = Math.max(0, e.attack - dt); const dist = player.x - e.x; if (e.hurt > 0) { e.vx *= 0.92; } else if (Math.abs(dist) < 260) { e.vx = Math.sign(dist) * 1.15; if (Math.abs(dist) < 42 && e.attack <= 0) { damagePlayer(6); e.attack = 54; } } else { e.vx *= 0.75; } e.x += e.vx * dt; } }
...
function draw() { ctx.clearRect(0,0,canvas.width,canvas.height); ctx.save(); const shake = feedback.shake > 0 ? Math.sin(world.timer * 2.7) * 3 : 0; ctx.translate(-world.camera + shake,0); drawWorld(); drawActors(); ctx.restore(); drawHud(); }
...
function drawEnemy(e){ ctx.fillStyle=e.hurt?'#fff':'#39414d'; ctx.fillRect(e.x,e.y,e.w,e.h); ctx.fillStyle=e.hurt?'#fff':'#b44'; ctx.fillRect(e.x,e.y-10,e.w,5); ctx.fillStyle='#3ee070'; ctx.fillRect(e.x,e.y-10,e.w*(e.hp/e.maxHp),5); if (e.hurt > 0) { ctx.fillStyle='#ffd45a'; ctx.fillText('HIT',e.x+5,e.y-18); } else if (Math.abs(player.x - e.x) < 70 && e.attack <= 8) { ctx.fillStyle='#ffd45a'; ctx.fillText('!',e.x+14,e.y-18); } }

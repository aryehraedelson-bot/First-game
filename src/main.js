...
const player = { name: 'Kai', age: 15, species: 'Golden Dragon', x: 90, y: 330, w: 34, h: 58, vx: 0, vy: 0, hp: 100, maxHp: 100, facing: 1, grounded: false, attacking: 0, combo: 0, comboWindow: 0, dodge: 0, lightning: false, lightningCd: 0, rescued: 0 };
...
function update(dt) {
  world.timer += dt; player.lightningCd = Math.max(0, player.lightningCd - dt); player.comboWindow = Math.max(0, player.comboWindow - dt); player.attacking = Math.max(0, player.attacking - dt); player.dodge = Math.max(0, player.dodge - dt);
  ...
  if (tapped('k') && !player.dodge) { player.dodge = 18; player.vx = player.facing * 9; }
...
function hitThings(range, dmg) { const ax = player.x + player.w / 2 + player.facing * range / 2; for (const e of enemies) if (e.alive && overlap(ax - range / 2, player.y, range, player.h, e.x, e.y, e.w, e.h)) { e.hp -= dmg; e.hurt = 8; if (e.hp <= 0) e.alive = false; } if (boss.active && !boss.defeated && overlap(ax - range / 2, player.y, range, player.h, boss.x, boss.y, boss.w, boss.h)) { boss.hp -= dmg; boss.hurt = 10; if (boss.hp <= 0) { boss.defeated = true; boss.active = false; objects.find(o => o.type === 'relic').active = true; world.objective = 'Absorb the Lightning Relic'; world.message = 'The Fire Beast falls. The Lightning Relic remains behind.'; } } }
function updateEnemies(dt) { for (const e of enemies) { if (!e.alive) continue; e.hurt = Math.max(0, e.hurt - dt); const dist = player.x - e.x; if (Math.abs(dist) < 260) { e.vx = Math.sign(dist) * 1.25; if (Math.abs(dist) < 42 && e.attack <= 0 && !player.dodge) { player.hp -= 6; e.attack = 50; } } e.attack -= dt; e.x += e.vx * dt; } }
function updateBoss(dt) { if (!boss.active || boss.defeated) return; boss.hurt = Math.max(0, boss.hurt - dt); const dist = player.x - boss.x; boss.vx = Math.sign(dist) * (Math.abs(dist) > 130 ? 1.45 : 0); boss.windup -= dt; if (boss.windup <= 0) boss.windup = 95; if (boss.windup < 18 && Math.abs(dist) < 150 && !player.dodge) player.hp -= 0.35 * dt; boss.x += boss.vx * dt; }
...
function drawKai(){ ctx.fillStyle='#d4a11e'; ctx.fillRect(player.x,player.y,player.w,player.h); ctx.fillStyle='#ffe28a'; ctx.fillRect(player.x+7,player.y-8,20,12); ctx.fillStyle='#f7d34d'; ctx.fillRect(player.x-3,player.y+16,player.w+6,30); ctx.strokeStyle=player.lightning?'#8ff3ff':'#e7bf49'; ctx.lineWidth=5; ctx.beginPath(); ctx.moveTo(player.x+17,player.y+28); ctx.lineTo(player.x+17+player.facing*(player.attacking?78:46),player.y+18); ctx.stroke(); if(player.lightningCd>70){ctx.strokeStyle='#9cf';ctx.lineWidth=3;ctx.strokeRect(player.x-12,player.y-12,player.w+24,player.h+24);} }
